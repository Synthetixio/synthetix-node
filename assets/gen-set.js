...
async function main() {
  for (const size of sizes) {
    await createResizedImage(inputFile, path.join(outputDir, `icon_${size}x${size}.png`), size);
    await createResizedImage(inputFile, path.join(outputDir, `icon_${size}x${size}@2.png`), size * 2);
  }
}
